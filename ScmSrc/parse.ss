...
(define (parse-datum e)
  ...
  (cond
   ...
   ((symbol? e)
    (ast-gen 'symbol e))
   ((vector? e)
    (ast-gen 'vector (map parse-datum (vector->list e))))
   ((pair? e)
    (ast-gen 'pair (parse-datum (car e))
	     (parse-datum (cdr e))))
   (else (error 'parse-datum "Unknown datum: " e))))
...
(define (parse-formal f-env e)
  ...
  ; returns: (b . a) where
  ; a is the ast for binding-occurrence of e,
  ; b is a binding for the symbol e, i.e., e->a.
  (if (symbol? e)
      (cond
       ((memq e syntactic-keywords)
	(error 'parse-formal "Illegal identifier (keyword): " e))
       ((lookup e f-env)
	(error 'parse-formal "Duplicate variable definition: " e))
       (else (let ((var-def-ast (ast-gen 'var-def e)))
	       (cons (gen-binding e var-def-ast)
		     var-def-ast))))
...
(define (parse-formal* formals)
  ...
  ;; returns (e . as) where:
  ;; as is a list (a1 ... an) of ast's for the formals (s1 ... sn)
  ;;    (in the corresponding order),
  ;; e is an environment { s1->a1, ... sn->an } for the formals s1...sn
  (define (pf* f-env asts formals)
    ;; f-env: "forbidden" environment (to avoid duplicate defs)
    ;; asts: the results of the parsing actions
    ;; formals: the unprocessed formals
    (if (null? formals)
	(cons f-env (reverse asts))
	(let* ((fst-formal (car formals))
	       (binding-ast (parse-formal f-env fst-formal))
	       (binding (car binding-ast))
	       (var-ast (cdr binding-ast)))
	  (pf*
	   (extend-env-with-binding f-env binding)
	   (cons var-ast asts)
	   (cdr formals)))))
  (pf* empty-env '() formals))
...
(define (parse-formals formals)
  ...
  ;; returns a pair (env . ast) where:
  ;; ast: is an ast for the formals
  ;; env: is the environment mapping symbols to the corresponding sub-ast's 
  ;;      of ast
  (letrec ((pfs (lambda (f-env formals)
		  (cond
		   ((null? formals)
		    (cons empty-env (ast-gen 'null-def)))
		   ((pair? formals)
		    (let* ((fst-formal (car formals))
			   ...
			   (renv-rast (pfs nf-env rem-formals))
			   (renv (car renv-rast))
			   (rast (cdr renv-rast)))
		      (cons
		       (extend-env-with-binding renv bind)
		       (ast-gen 'pair-def res rast))))
		   (else
		    (let* ((bind-ast (parse-formal f-env formals))
			   (bind (car bind-ast))
			   (ast (cdr bind-ast)))
		      (cons
...
(define (parse-expression* env exprs)
  ...
  (letrec ((pe*
	    (lambda (asts es)
	      (cond
	       ((null? es) asts)
	       ((pair? es) (pe* (cons (parse-expression env (car es)) asts) (cdr es)))
	       (else (error 'parse-expression* "Not a list of expressions: " es))))))
    (reverse (pe* '() exprs))))


; parse-arguments

(define (parse-arguments env exprs)
  ;; parses lists of arguments of a procedure call
  (cond
   ((null? exprs) (ast-gen 'null-arg))
   ((pair? exprs) (let* ((fst-expr (car exprs))
			 (rem-exprs (cdr exprs))
			 (fst-ast (parse-expression env fst-expr))
			 (rem-ast (parse-arguments env rem-exprs)))
		    (ast-gen 'pair-arg fst-ast rem-ast)))
   (else (error 'parse-arguments "Illegal expression list: "
		exprs))))
...
(define (parse-variable env e)
  (if (symbol? e)
	  ...
	  (let ((assoc-var-def (lookup e env)))
	    (if assoc-var-def
		(ast-gen 'variable (binding-value assoc-var-def))
...
(define (parse-procedure-call env op args)
  (ast-gen 'procedure-call
	   ...
	   (parse-arguments env args)))


; parse-quote

(define (parse-quote env args)
  (if (list-of-1? args)
...
(define (parse-lambda env args)
  (if (pair? args)
      (let* ((formals (car args))
	     (body (cdr args))
	     (nenv-fast (parse-formals formals))
	     (nenv (car nenv-fast))
	     (fast (cdr nenv-fast)))
...
(define (parse-body env body)
  ...
  (define (def-var f-env clause)
    ...
    (if (pair? clause)
        ...
        #f))
  (if (pair? body)
      (parse-command* (def-var* env body) body)
...
(define (parse-named-let env args)
  ...
  (if (pair? (cdr args))
      (let* ((variable (car args))
	     (bindings (cadr args))
	     (body (cddr args))
	     (vbind-vast (parse-formal empty-env variable))
	     (vbind (car vbind-vast))
	     (vast (cdr vbind-vast))
	     (env-ast (parse-parallel-bindings env bindings))
	     (nenv (car env-ast))
	     (basts (cdr env-ast)))
...
(define (parse-binding f-env env binding)
  ; parses a single binding
  ; returns: (v . e) where:
  ; v is a variable definition ast, 
  ; e is an expression ast;
  ; f-env: a forbidden environment
  ; env: the environment in which binding is to be parsed
  ; binding: a pair of the form (<variable> <expression>)
  (cons (parse-formal f-env (car binding))
	(parse-expression env (cadr binding))))


; parse-parallel-bindings

(define (parse-parallel-bindings env bindings)
  ; returns (env . (v . e)*) where:
  ; env is the environment generated by bindings,
  ; (v . e)* is a list of pairs of variable definition/expression pairs
  (define (ppb f-env env basts bindings)
    (if (null? bindings)
	(cons f-env (reverse basts))
	(let* ((fst-binding (car bindings))
	       (rem-bindings (cdr bindings))
	       (formal-expr (parse-binding f-env env fst-binding))
	       (fast (cdar formal-expr))
	       (expr (cdr formal-expr)))
	  (ppb (extend-env-with-binding f-env (caar formal-expr))
	       env 
	       (cons (cons fast expr) basts)
	       rem-bindings))))
  (if (list-of-list-of-2s? bindings)
      (ppb empty-env env '() bindings)
...
(define (parse-sequential-bindings env bindings)
  ; returns a pair consisting of the local, new environment
  ; and a list of pairs (<var-def-ast . <expression-ast>)
  (define (psb f-env env basts bindings)
	...
	(let* ((fst-binding (car bindings))
	       ...
	       (fast (cdar formal-expr))
	       (expr (cdr formal-expr)))
	  (psb (extend-env-with-binding f-env (caar formal-expr))
	       (extend-env-with-binding env (caar formal-expr))
	       (cons (cons fast expr) basts)
	       rem-bindings))))
  (if (list-of-list-of-2s? bindings)
      (psb empty-env env '() bindings)
...
(define (parse-recursive-bindings env bindings)
  ...
  ; and a list of pairs (<var-def-ast . <expression-ast>)
  (if (list-of-list-of-2s? bindings)
      (let* ((fenv-fasts (parse-formal* (map car bindings)))
	     (fenv (car fenv-fasts))
	     (fasts (cdr fenv-fasts))
	     (expr-asts (parse-expression*
			 (extend-env-with-env env fenv)
			 (map cadr bindings))))
	(cons
	 fenv
	 (map cons fasts expr-asts)))
...
(define (parse-command env c)
  (if (pair? c)
      (let ((op (car c))
	    (args (cdr c)))
	(case op
	  ((define) (parse-define env args))
	  ((begin) (ast-gen 'begin-command (parse-command* env args)))
	  (else (parse-expression env c))))
      (parse-expression env c)))


; parse-command*

(define (parse-command* env commands)
  ;; parses a sequence of commands
  (if (list? commands)
      (map (lambda (command) (parse-command env command)) commands)
