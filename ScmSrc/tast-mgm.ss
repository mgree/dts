...
(define (ast-gen . args)
  ...
  ; -> applied occurrences of variables are of the form
  ; (c o v . cs) (with o either 'variable or 'identifier) representing
  ; [c] v[cs]
...
(define (ast-type ast)
  ;; extracts the ("high") type (type with coercion applied) of ast 
...
(define (tast-check! tast)
  ...
  ; type equalities required by the typing rules to the global constraint set
  (let ((op (ast-con tast))
	(args (ast-args tast))
    ...
    (case op
      ...
      ((identifier) (let ((id-binding (lookup (fst args) top-level-env)))
		      (if id-binding
			  (add-equality-constr! low-type (instantiate-type 
						 (binding-value id-binding)))
			  (set! top-level-env (extend-env-with-binding
					       top-level-env
...
(define (typed-program-check! p)
  ;; type checks a whole program; i.e., a list of tast's
	  ...
	  (case op
	    ...
	    ((variable) (let* ((assoc-var-def (fst args))
			  ...
			  (if (null? coercion-list)
			      thesymbol
			      (list thesymbol (map coercion-show 
						   coercion-list)))))
...
(define (unparse-typed-program asts)
  ;; unparses an abstract syntax representation of a typed program into a
