structure KernelAttributes =
  ...
  fun literal (_, d: 'a datum) (Env:parameter env): aexp = 
                    LITERAL (new_coercion(), make_datt d)
  fun variable (_,s: string) (Env: parameter env): aexp = 
        VARIABLE (new_coercion(), 
                  (lookup s Env handle Lookup => FREE (s, new_typevar()),
                   ref []))
  fun call (_, e: aexpf ,l: aargsf) (Env: parameter env): aexp = 
                   CALL (new_coercion(), e Env, l Env)
  fun lambda (_,f: ftrip,e: aexpf) (Env: parameter env): aexp = 
        let val (st: aforms,fenv: parameter env, c: coercion) = f 
	    val nt = new_typevar()
	    val nt' = new_typevar()
            val dom = func(hi_type c,nt)
        in (debug("make_attributes :", "before calling equiv on lambda ...\n");
            equiv(nt', dom); 
(*
            equiv(hi_type c, lo_type c);
            equiv(nt', func(lo_type c, nt)); 
*)
            debug("make_attributes :", "after calling equiv on lambda ...\n");
            LAMBDA ((COMP ((MAP (FUNC, [c,(IDC,nt,nt)]), 
                           func(hi_type c,nt), 
                           func(lo_type c,nt)), 
		           make_coercion(dom ,nt')), 
                           dom, nt'),
	            st, e (add (Env, fenv))))
        end
  ...
  fun apairpar (_,s:string,f:ftrip): ftrip = 
	  ...
	  val new_par = LAMBOUND (s, nt)
	  val (st, env, c) = f
          val pairty1 = pair(nt,lo_type c)
          val pairty2 = pair(nt', hi_type c) 
      in 
      (lookup s env; raise StaticAnalysisError "Duplicate parameter")
       handle Lookup => 
          (debug("make_attributes :", "before calling equiv on pairpar  ...\n"); 
           equiv(nt', pairty1); 
(*         equiv(lo_type c, hi_type c);
           equiv(nt', pair(nt, lo_type c));
*)
           debug("make_attributes :","after calling equiv on pairpar ...\n"); 
           (APAIRPAR ((),new_par,st), 
           extend (env, s, new_par),
	   (COMP (make_coercion(nt', pairty1), 
	              (MAP (PAIR, [(IDC,nt,nt), c]), 
                       pair(nt,lo_type c), 
                       pair(nt, hi_type c))),
