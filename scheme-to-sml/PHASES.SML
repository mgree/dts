  ...
  val empty_atts = INIT{ parameter = empty,
                         variable = empty,
                         exp = fn () => "@" }
  ...
  val (empty_parse_exp, empty_parse_def, empty_parse_program) = 
      read empty_atts;
  end;
  ...
  val evar_hom = EHOM { noexp = noexp,
                   ...
                   nullarg = nullarg}
  in
  val make_attributes_exp = apply_ehom evar_hom

  fun make_attributes_def (DEFINE ((s, ()), e)) Env =
        let val nt = new_typevar()
...
(* 4th stage: Collect constraints from source program *)
  ...
  in
  val get_constraint_exp = apply_ehom constraints_hom
...
(* 6th stage: Set predecessor/successor fields in constraints *)
  ...
  in
  val reset_pred_succs =
      foreach (fn (t1,t2) => (get #preds t2 := nil; get #numpreds t2 := 0;
			                  get #succs t1 := nil; get #numsuccs t1 := 0))
...
val reset_shade = foreach (fn t => get #color t := WHITE)
end;
  ...
  | propagate_sources (at :: r) =
      let val _ =
		     ...
		     then initialize_source at
	               ...
	               get #neg at :=
			           exists (fn pt => !(get #neg pt)) (!(get #preds at)))
          val newsrcs =
	         fold (fn (at, newsrcs) => 
  ...
  | propagate_sinks (at :: r) =
      let val _ =
	 		 ...
	 		 then initialize_sink at
				     ...
				     fold (fn (pt, env) => or_combine (!(get #sinks pt), env))
			    	      (!(get #succs at))
			        	  empty_env;
	           	   get #pos at :=
			      	 exists (fn st => !(get #pos st)) (!(get #succs at)))
          val newsnks =
	        fold (fn (at, newsnks) => 
       ...
       then ()
             ...
             | TVAR _ =>
                 ...
                 | (false, false) =>
	                  ...
	                  val cienv = contract ienv
		      in case keys cienv of
	                           nil => link(t, unspec())
...
in
fun new_name () =
    "REC" ^ Int.string (!rec_counter) before rec_counter := !rec_counter + 1
end
...
fun makelst t = make_type (LST, [t])
fun find_list_type t =
    let val color = get #color t 
    in case !color of 
    	WHITE => (color := GREY;
				  (case utype t of
				  ...
				  | SIMPLE (PAIR, [t1,t2]) =>
					if equal(t,t2)
                                        then (debug("find_list_type",
						    "list type found");
					      link(t, makelst t1);
					      find_list_type t1)
				        else (find_list_type t1;
					      find_list_type t2)
				  | SIMPLE (_, tlist) => 
				  	   apply find_list_type tlist
				  | DYN f => 
				       apply find_list_type (summands f));
				  color := BLACK)
	  | GREY => ()
	  | BLACK => ()
    end
...
fun find_list_types N =
    (foreach (fn t => get #color t := WHITE) N;
    foreach find_list_type N)
end;
...
val topenv = list2env
    [ ("cons", let val nt1 = new_typevar()
      ...
      ("null?", let val nt1 = new_typevar()
                    val nt2 = new_typevar()
      		    in close ([], funcl (
                             [make_dyn_type [unit(),pair(nt1,nt2) ]], bool()))
      		    end),
      ("inc", let val nt1 = new_typevar()
                  val nt2 = new_typevar()
              in close ([], funcl ([number()], number()))
              end),
      ("equal?", let val nt = new_typevar()
                 in close ([], funcl ([nt, nt], bool()))
                 end)]
end


(* Alternative type for null?
