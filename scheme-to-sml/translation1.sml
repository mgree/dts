...
in
  ...
  datatype syncode = CONSTR | DESTR | NEUT

  datatype ccode = CID | CCVAR | CTAG | CCHECK | CERR | L2P | P2L
...
*)
  ...
  exception IsrecursiveSum;
  fun isrecursive_sum t =
        case utype t of
          DYN f => fold (fn (x, y) => (isrecursive x) orelse y) 
                        (summands f) 
                        false
        | _ => raise IsrecursiveSum 
...
*)
      ...
      | (SIMPLE(tt, _), SIMPLE(tt',_)) =>
           if tt = tt' then (mk_CID(), CID)
           else 
                (* paren(mk_ERR()) *) (mk_ERR(), CERR)

     |  (SIMPLE(tt, _), DYN _) => (paren(mk_TAG tt), CTAG)

     |  (DYN _, SIMPLE (tt, _)) => (paren(mk_CHECK tt), CCHECK)

     |  (TVAR _, _) => 
           if equal(t1, t2) then (mk_CID(), CID)
           else (mk_CVARexp (new_cv(t1, t2)), CCVAR)
    ...
    |  _ =>  (mk_CID(), CID)
      ...
      *)
  fun TCapp(code, s as (t1, t2), e) =
          if isid s  then 
                       case code of
                         DESTR => if isrecursive t1 
                                  then mk_APPexp(mk_UNFOLD(), e) 
                                  else e
                       | CONSTR => if isrecursive t2 
                                   then mk_APPexp(mk_FOLD(), e)
                                   else e
                       | NEUT => e
          else
               let val (cexp, Ccode) = Tcoercion s
               in
                   case Ccode of   
                     CTAG => if isrecursive_sum t2 then mk_APPexp(mk_FOLD(), mk_APPexp(cexp, e))
                                else mk_APPexp(cexp, e)
                   | CCHECK => if isrecursive_sum t1 then mk_APPexp(cexp, mk_APPexp(mk_UNFOLD(), e))
                               else mk_APPexp(cexp, e)
                   | _ => mk_APPexp(cexp, e)
  ...
  fun Tdatum d =
      let  fun str2int s = 
           ...
           fun booldat (a, b)      = TCapp(CONSTR, a, ATEXPexp((), mk_IDENTatexp((), 
                                              Bool.string b, false)))
           fun chardat (a, c)      = TCapp(CONSTR, a, ATEXPexp((),SCONatexp ((), 
                                              SCon.STRING c)))
           fun stridat (a, s)      = TCapp(CONSTR, a, ATEXPexp((), SCONatexp ((), 
                                              SCon.STRING s)))
           fun symbdat (a, s)      = TCapp(CONSTR, a, ATEXPexp((), SCONatexp ((), 
                                              SCon.STRING s)))
           fun numbdat (a, n)      = TCapp(CONSTR, a, ATEXPexp((), SCONatexp ((), 
                                              SCon.INTEGER (str2int n))))
           fun vectdat x           = raise TranslationUnimplemented
           fun pairdat (a, d1, d2) = TCapp(CONSTR, a, ATEXPexp((), 
                                              mk_pair_atexp((), d1, d2)))
           fun nildat a            = TCapp(CONSTR, a, ATEXPexp((), 
                                              LISTatexp((), [])))
  ...
  fun Texp e  =
          ...
          fun variable(ea, (s, tyscheme)) = 
             ...
             *)
                ...
                in
                    case tyscheme of
                      TYPE _ => TCapp(NEUT, ea, varexp) 
                                           (* no quantified *)
                    | TSCHEME(siglist, _) =>
                        (case siglist of
                           nil => TCapp(NEUT, ea, varexp) 
                                             (* no quantified *)
                         | [s] => (* one quantified *)
                                  let val (coercion, Ccode) = Tcoercion s 
                                      val appexp = mk_APPexp(varexp, 
                                                             coercion)
                                  in
                                      TCapp(NEUT, ea, appexp)
                                  end
                         | _ => (* more than one quantified *)
                                let val coercions = map (#1 o Tcoercion) siglist
                                    val appexp = mk_APPexp(varexp,
                                                          ATEXPexp((),
                                                          NTUPLEatexp((),
                                                          coercions)))
          ...
          fun call(ea, e, l)  = TCapp(NEUT, ea, mk_APPexp(e, l))
          fun lambda(ea, f, e)   = 
              let val mrule = MRULE(dummy_info(), f, e)
                  val match = MATCH(dummy_info(), mrule, None)
              in
                  TCapp(CONSTR, ea, paren(FNexp((), match)))
              end 
          fun ifexp(ea, e, e', e'')  = TCapp(NEUT, ea, paren(IFexp((), e, e', e''))) 
...
in
 ...
 fun Tann(code, (t1, t2), e) =
        mk_APPexp(mk_ANN(ty2str t1, ty2str t2), e)
  ...
  fun TdatumAnn d =
      let  fun str2int s = 
           ...
           fun booldat (a, b)      = Tann(CONSTR, a, ATEXPexp((), mk_IDENTatexp((), 
                                              Bool.string b, false)))
           fun chardat (a, c)      = Tann(CONSTR, a, ATEXPexp((),SCONatexp ((), 
                                              SCon.STRING c)))
           fun stridat (a, s)      = Tann(CONSTR, a, ATEXPexp((), SCONatexp ((), 
                                              SCon.STRING s)))
           fun symbdat (a, s)      = Tann(CONSTR, a, ATEXPexp((), SCONatexp ((), 
                                              SCon.STRING s)))
           fun numbdat (a, n)      = Tann(CONSTR, a, ATEXPexp((), SCONatexp ((), 
                                              SCon.INTEGER (str2int n))))
           fun vectdat x           = raise TranslationUnimplemented
           fun pairdat (a, d1, d2) = Tann(CONSTR, a, ATEXPexp((), 
                                              mk_pair_atexp((), d1, d2)))
           fun nildat a            = Tann(CONSTR, a, ATEXPexp((), 
                                              LISTatexp((), [])))
  ...
  fun TexpAnn e  =
          ...
          fun variable(ea, (s, tyscheme)) = 
             ...
             *)
                ...
                in
                    case tyscheme of
                      TYPE _ => Tann(NEUT, ea, varexp) 
                                           (* no quantified *)
                    | TSCHEME(siglist, _) =>
                        (case siglist of
                           nil => Tann(NEUT, ea, varexp) 
                                             (* no quantified *)
                         | [s] => (* one quantified *)
                                  let val (coercion, Ccode) = Tcoercion s 
                                      val appexp = mk_APPexp(varexp, 
          ...
          fun call(ea, e, l)  = Tann(NEUT, ea, mk_APPexp(e, l))
          fun lambda(ea, f, e)   = 
              let val mrule = MRULE(dummy_info(), f, e)
                  val match = MATCH(dummy_info(), mrule, None)
              in
                  Tann(CONSTR, ea, paren(FNexp((), match)))
              end 
          fun ifexp(ea, e, e', e'')  = Tann(NEUT, ea, paren(IFexp((), e, e', e''))) 
          fun assign(ea, v, e)    = raise TranslationUnimplemented
                                    (* right-assocviative argument tuple *)
          fun pairarg(la, e, l)   = ATEXPexp((), NTUPLEatexp((),[e,l])) 
          fun nullarg la = ATEXPexp((),LISTatexp((),[]))
...
in
  ...
  fun TdefAnn (DEFINE((defvar, _), e)) =
        (reset_cvs(); (* CRUCIAL: initialize cvs *)
        let val exp = TexpAnn e (* coercion environment cvs
                                gets extended to contain just
                                the coercion variables (indices) 
                                occurring inside e. Ultimately, the 
                                extension occurs in function
  ...
  fun TprogramAnn p =
      let val programdecs    = map TdefAnn p
      in
         mk_SEQdec programdecs
      end
...
fun translate_programAnn(p, Env) =
    case p of
      [] => []            
    | d::p' => let val ad as (* type a single definition *) 
                        (DEFINE((name, ty), body)) = 
                     ...
                     val Env' = (output(std_out, "\n calling extend...\n");
                                 extend(Env, name, tyscheme)) 
                 in  (output(std_out, "\n calling translate...\n");
                     SMLdef :: (translate_programAnn(p', Env')))
                 end
  ...
  fun scheme2smlAnn instream =
      let val p = parse_program instream (* parse input stream into  
                                    preattributed definition list *)
                      (* translate to SML AST *)
          val SMLdecs = translate_programAnn(p, topenv)
      in  
          (* send AST to pretty printer *)
          (PPdeclst SMLdecs;
  ...
  fun TAnn s = let val ip = open_in s
            in
                scheme2smlAnn ip
            end
