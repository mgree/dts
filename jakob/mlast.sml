...
in
  ...
  fun paren e = ATEXPexp((), PARatexp((), e))

      (* construct ML-application (e e') from ML-exps e,e' *)
  fun mk_APPexp(e, e') =  paren(APPexp(dummy_info(), e, EXPatexp((),e')))
  ...
  fun mk_tupleFNexp(vlist, e) =
      let val patlist = map (fn vname => mk_ATPATpat((), vname)) 
                            vlist
          val pat = ATPATpat((), NTUPLEatpat((), patlist))
          val mrule = MRULE((), pat, e)
          val match = MATCH((), mrule, None)
     in
          FNexp((), match)
     end
  ...
  fun mk_simpleRECVALdec(valname, e) =
       VALdec((), RECvalbind((), mk_simplePLAINvalbind(valname, e))) 
...
(* out2
  ...
  val check_decs = map mk_check_dec type_tags
...
(* construction of coercion expressions *)
  ...
  fun mk_ANN(s1, s2) = ATEXPexp((), mk_IDENTatexp((), "<"^s1^","^s2^">", false))


  fun mk_FOLD() = ATEXPexp((), mk_IDENTatexp((), "FOLD", false))
  fun mk_UNFOLD() = ATEXPexp((), mk_IDENTatexp((), "UNFOLD", false))


      (* make the error coercion fn x => raise TypeError *)

(*
  fun mk_ERR() =
      let val e = mk_RAISEexp "TypeError"
      in
          mk_simpleFNexp("x", e)
      end
*)

  fun mk_ERR() = ATEXPexp((), mk_IDENTatexp((), "ERR", false))
  ...
  fun mk_map(type_tag, c, d, e) = 
      let  fun mk_arrow_coercion(c,d,e) = (*  (c -> d)  *)
               let val x        = fresh_varname()
                   val idx      = mk_IDENTatexp((), x, false)
                   val app_cx   = mk_APPexp(c, ATEXPexp((),idx))
           ...
           fun mk_pair_coercion(c,d,e) =  (*  (c * d)  *)
               ...
               in
                   mk_pair_exp(mk_APPexp(c, proj1_e), 
                               mk_APPexp(d, proj2_e))
               end
      in
           case type_tag of
