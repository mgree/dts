; ---------------------------------------------------------------------------
; Parsing and abstract syntax definition for IEEE Standard Schem
; ---------------------------------------------------------------------------
;; Needed packages: environment managemen
;(load "env-mgmt.ss")
; Needed routines: ast-gen, ast-con, ast-arg
;; Abstract synta
;
;; VarDe
;
...
;; null-def:            ()                -> Formals
;; pair-def:            (VarDef Formals)  -> Formal
;;
;; Expr
;;
;; Datum < Exp
...
;; Args < Expr
;;
;; variable:		(VarDef)                  -> Variabl
;; identifier:		(Symbol)       		-> Variabl
;; null-arg:            ()                        -> Ar
...
;; cond-expression:	(CondClause+)	        -> Exp
;; case-expression:	(Expr CaseClause*) 	-> Exp
;; and-expression:	(Expr*)			-> Expr
;; or-expression:	(Expr*)			-> Exp
;; let-expression:	(Binding* Expr+) -> Expr
;; named-let-expression: (VarDef Binding* Expr+) -> Expr
;; let*-expression:	(Binding* Expr+)  -> Exp
;; letrec-expression:	(Binding* Expr+)  -> Exp
...
;; Definition < Expr
;; 
;; definition:		(Variable Expr)	        -> Definitio
;; function-definition: (Variable Formals Expr+)  -> Definition
;; begin-command:	(Definition*)		-> Definitio
;
;
;; Program =		Expr
...
(define (parse-datum e)
  ;; Source: IEEE Scheme, sect. 7.2, <datu
...
; parse-formal: parses nonterminal <variable> in defining occurrence positio
...
(define (parse-formals formals)
  ;; parses <formals>; see IEEE Scheme, sect. 7.
  ;; returns a pair (bindings . ast) where
  ;; ast: is an ast for the formal
  ;; bindings: is a list of bindings mapping the identifier symbols to the 
  ;; corresponding variable-definition ast
  (cond
   ((null? formal
...
(define (parse-begin env arg
  (ast-gen 'begin-expressio
	   (parse-body env args))
; parse-cond
...
(define (parse-named-let env args
  ...
  (if (pair? (cdr args)
      (let* ((variable (car args))
	     ...
	     (nenv (car env-ast)
	     (basts (cdr env-ast))
	(ast-gen 'named-let-expressio
		 vast bast
		 (parse-body (extend-env-with-en
...
(define (parse-binding env binding)
  ...
  ; i is identifier foun
  ; v is a variable definition ast for i, 
  ; e is an expression ast bound to v;
  ; env: the environment in which binding is to be pars
...
(define (parse-parallel-bindings env bindings)
  ; returns ((i . v)* . (v . e)*) where:
  ; (i . v)* is the set of bindings mapping identifier i to variable ast 
  ; (v . e)* is the set of bindings mapping variable ast's to bound exp ast'
...
(define (parse-sequential-bindings env bindings)
  ...
  (if (list-of-list-of-2s? bindings
      (psb '() env '() bindings)
      (error 'parse-sequential-binding
	     "Not a list of bindings: " bindings)))

; parse-letrec
(define (parse-letrec env arg
  (if (pair? args)
      (let* ((bindings (car args))
	     (body (cdr args))
	     ...
	     (nenv (car env-ast))
	     (basts (cdr env-ast)))
	(ast-gen 'letrec-expressio
		 basts
		 (parse-body (extend-env-with-env env nenv) body))
      (error 'parse-letrec "Illegal bindings/body: " args)
; parse-recursive-binding
...
; writ
(define (write* l port)
  ;; writes a list l of S-expressions to a por
...
(define (input-program file-name)
  ;; file-name is a string containing a Scheme source code representatio
