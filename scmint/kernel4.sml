...
fun Cadlst adlst =
    ...
    | ((h,l), APAIRDAT(ad, adlst')) =>
              ...
              in
                  (unify (l, consty);    (* l = (cons ad_h adlst_h) *)
                   inC (l,h);         (* l < h *)
                   (emptyEnv, h))
              end
...
and Cadilist adilist =
   ...
   | ((h,l), AIPAIRDAT (ad, adilist')) => 
            ...
            in
                (unify (l, consty);    (* l = (cons ad_h adilist_h) *)
                 inC (l,h);         (* l < h *)
                 (emptyEnv, h))
            end
...
(* unify all variables in the intersection of A and A' *)
    ...
    | (var, ann)::tl => if inDom var A' 
                        then (unify (ann, lookup var A'); tl AINTER A')
                        else tl AINTER A'
...
fun Cannfmls annfmls A = 
    ...
    | ((h,l), AVARPAR ((h',l'), var)) =>
              (unify (h', l');    
               unify (l, h);   
               unify (l, h'); 
               if inDom var A then unify (lookup var A, l') else ())                        
...
fun Cargs annargs  =
    ...
    | ((h,l), PAIRARG (a, args')) =>
              ...
              in
                  (unify (l, consty);    (* l = (cons a_h args'_h) *)
                   inC (l,h);
                   (A AAND A', h))
              end
...
and Cexp annexp  = 
    ...
    | ((h,l), CALL(rator, args)) =>
              ...
              in
                  (inC (l,h);            (* l < h *)
                   unify (rator_h, arrowtype); (* rator_h = args_h -> l *)
                   (A AAND A', h))
              end
...
fun pred_succ (l,h) =
    if equal (l, h) 
       then ()
    else let val pred_h = get_preds h
             val succ_l = get_succs l
         in (pred_h := l :: (!pred_h);
             succ_l := h :: (!succ_l))
         end
