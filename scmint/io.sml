structure IO: IO =
  ...
  val input_port_setting = ref std_i
  ...
  fun typecase {bool = bool_fcn
      ...
      case dval of
	  ref (BOOL b) => bool_fcn 
        | ref (SYMBOL s) => symbol_fcn 
	| ref (CHAR c) => char_fcn c
	| ref (VECTOR v) => vector_fcn 
	| ref (PAIR p) => pair_fcn 
	| ref (NUMBER n) => number_fc
	| ref (STRING s) => string_fcn s
	| ref (PROCEDURE p) => procedure_fcn 
	| ref (LIST l) => list_fcn l
	| ref (INPUT_PORT iport) => input_port_fcn iport
	| ref (OUTPUT_PORT oport) => output_port_fcn opor
	| dval => otherwise_fcn dva
  i
  fun read ip 
	  ...
	  and read_vector() =
		...
		| _ => read_datum(tok) :: read_vector()
	      en
      i
          let val tok = get_next_token() i
              if tok = EndOfInput then CHAR_TAG eof_ch
              else read_datum(tok)
          en
      en
  ...
  fun write (dval,oport) 
            le
               ...
               |   char_rep1 " "  = "#\\space"
               |   char_rep1 c    = "#\\"^c
               fun is_empty_list x = if is_list x then is_null x else fals
               fun rep dval 
                       let val err = fn _ =>  "()
  ...
  fun display (dval,oport) 
            let fun disp dval =
               ...
               and symbol_disp s = symbol2strin
               and char_disp c   = char2str 
               and vector_disp v  = "#("^(strlist2string(map disp (vector2list v)))^")
               and pair_disp (d1,d2) = "("^(disp d1)^(pair_disp1 d2)^")
               and number_disp n = number2str 
               and string_disp ss = mstring2string s
               and list_disp l = "("^(strlist2string(map disp l))^")
               and pair_disp1 
                              if is_null d then ""
                              els
                                 if is_pair d the
                                    let val (p1,p2) = PAIR_UNTAG 
                                    i
                                        " "^(disp p1)^(pair_disp1 p
                                    e
                                 els
                                    " . "^(disp 
           i
