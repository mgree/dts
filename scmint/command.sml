structure Command: COMMAND =
  ...
  fun is_keyword "quote" = true
    ...
    | is_keyword "let" = true
    | is_keyword "let*" = true
    | is_keyword "letrec" = true
    | is_keyword "do" = true
    | is_keyword "quasiquote" = true
  ...
  fun parse_exp (d as (PAIRDAT (d1, d2))) =
	...
	SYMBDAT "quote" => parse_quote (d, d2)
      ...
      | SYMBDAT "begin" => 
	  (case parse_begin (d, d2) of
	     DEF def => raise ParseError ("Illegal begin expression", d)
	   | EXP exp => exp)
      | SYMBDAT "cond" => 
	  (case parse_cond_clauses d2 of
      ...
      | SYMBDAT "letrec" => parse_letrec (d, d2)
    ...
    | parse_exp (SYMBDAT s) = VARIABLE (parse_identifier s)
    ...
    | parse_exp d = LITERAL d
  and parse_quote (d, PAIRDAT (d', NILDAT)) = LITERAL d'
    | parse_quote (d, _) = raise ParseError ("Illegal quotation", d)
  and parse_lambda (d, PAIRDAT (f, b)) =
        LAMBDA (parse_formals f, parse_body b)
    | parse_lambda (d, _) = raise ParseError ("Illegal lambda expression", d)
  ...
  and parse_command (d as (PAIRDAT (SYMBDAT "define", d'))) = 
    ...
    | parse_command d = EXP (parse_exp d)
  ...
  and parse_define (d, PAIRDAT (SYMBDAT s, PAIRDAT (d', NILDAT))) =
	...
	FUNDEF (parse_identifier s, parse_def_formals f, parse_body b)
  ...
  and parse_defs (PAIRDAT (d1, d2)) = 
    ...
    | parse_defs NILDAT = nil
    | parse_defs d = raise ParseError ("Illegal definitions", d)
  and parse_exps NILDAT = nil
    | parse_exps d = parse_seq d
  ...
  and parse_cond_clauses NILDAT = NULLCOND
  ...
  and parse_case_clauses NILDAT = NULLCASE
  ...
  and parse_iter_specs NILDAT = nil
	...
	parse_iter_spec d1 :: parse_iter_specs d2
  ...
  and parse_template_or_splice 1 
