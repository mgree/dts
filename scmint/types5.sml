(* signature KERNELTYPES =
  ...
  val make_dyn_type: atype list -> atype
...
structure KernelTypes =
  ...
  datatype type_tag = FUNC | BOOL | NIL | PAIR
  val type_tags = [FUNC, BOOL, NIL, PAIR]
    ...
    | propagate (tp::rp, ln) =
        ...
        in if !posptr then 
              propagate (rp, ln)
                ...
                (case (#1 (!!tp)) of
                   TVAR _ => 
                     let val preds = !(#preds tatts)
                     in propagate (preds @ rp, ln)
                     end
                 | SIMPLE (FUNC, [td,tr]) =>
       ...
       let val tatts = attributes tn
        ...
        in if !negptr then 
              propagate ([], rn)
                ...
                (case (#1 (!!tn)) of
                   TVAR _ => 
                     let val succs = !(#succs tatts)
                     in propagate ([], succs @ rn)
                     end
                 | SIMPLE (FUNC, [td,tr]) =>
                     propagate ([td], tr::rn)
                 | SIMPLE (_, tlist) =>
  ...
  fun interpret t =
    if !(get #interpreted t) then () else
